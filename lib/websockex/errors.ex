...
defmodule Websockex.CallbackError do
  defexception [:reason, :stacktrace, :function, :args]

  def message(%__MODULE__{reason: reason, stacktrace: stacktrace, function: function, args: args}) do
    [state | args] = Enum.reverse(args)
    args = Enum.reverse(args)

    """
    Websockex #{inspect(self())} terminating
    #{Exception.format_banner(:error, reason, stacktrace)}
    #{Exception.format_stacktrace(stacktrace) |> String.trim_trailing()}
    Function:  #{function}
